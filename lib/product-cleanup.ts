...
export const cleanupSoldOutProducts = async (): Promise<CleanupResult> => {
	try {
		console.log(
			"🧹 [Product Cleanup] Starting automatic cleanup of sold-out products...",
		);
		...
		if (lastCleanup) {
			...
			if (hoursSinceLastCleanup < CLEANUP_INTERVAL_HOURS) {
				console.log(
					`🧹 [Product Cleanup] Skipping cleanup - last run was ${hoursSinceLastCleanup.toFixed(1)} hours ago`,
				);
		...
		if (fetchError) {
			console.error(
				"🧹 [Product Cleanup] Error fetching sold-out products:",
				fetchError,
			);
		...
		if (!soldOutProducts || soldOutProducts.length === 0) {
			console.log(
				"🧹 [Product Cleanup] No sold-out products found for cleanup",
			);
		...
		console.log(
			`🧹 [Product Cleanup] Found ${soldOutProducts.length} sold-out products to remove`,
		);
		...
		if (cartCleanupError) {
			console.error(
				"🧹 [Product Cleanup] Error cleaning up cart items:",
				cartCleanupError,
			);
		...
		if (deleteError) {
			console.error(
				"🧹 [Product Cleanup] Error deleting sold-out products:",
				deleteError,
			);
		...
		console.log(
			`🧹 [Product Cleanup] Successfully removed ${soldOutProducts.length} sold-out products`,
		);
		...
		console.error(
			"🧹 [Product Cleanup] Unexpected error during cleanup:",
			error,
		);
		return {
			success: false,
