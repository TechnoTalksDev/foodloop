...
export const NotificationProvider: React.FC<PropsWithChildren> = ({
	...
	const getPreferencesKey = (userId: string) =>
		`notification_preferences_${userId}`;
	const getLastCheckKey = (userId: string, type: string) =>
		`last_check_${type}_${userId}`;
	...
	const checkPlantReminders = useCallback(async () => {
		...
		try {
			...
			const lastCheck = await AsyncStorage.getItem(
				getLastCheckKey(session.user.id, "plants"),
			);
	...
	const checkWeatherAlerts = useCallback(async () => {
		...
		try {
			...
			const lastCheck = await AsyncStorage.getItem(
				getLastCheckKey(session.user.id, "weather"),
			);
			...
			if (Math.random() < 0.05) {
				...
				addNotification({
					...
					message:
						"Temperatures may drop below freezing tonight. Protect your sensitive plants.",
	...
	const checkMarketplaceUpdates = useCallback(async () => {
		...
		try {
			// Check if we've already checked recently
			const lastCheck = await AsyncStorage.getItem(
				getLastCheckKey(session.user.id, "marketplace"),
			);
			...
			console.log(
				`🛒 [Marketplace] New items notification sent (${products.length} items)`,
			);
		} catch (error) {
	...
	const checkCommunityUpdates = useCallback(async () => {
		...
		try {
			// Check if we've already checked recently
			const lastCheck = await AsyncStorage.getItem(
				getLastCheckKey(session.user.id, "community"),
			);
            ...
            `,
					...
					if (newReplies && newReplies.length > 0) {
						for (const reply of newReplies) {
							const authorName =
								reply.author?.name || reply.author?.username || "Someone";
            ...
            `,
					...
					if (newVotes && newVotes.length > 0) {
						...
						const votesByPost = newVotes.reduce(
							(
								acc: Record<
									number,
									{ ups: number; downs: number; title: string }
								>,
								vote: any,
							) => {
								if (!acc[vote.post_id!]) {
          ...
          `,
            ...
            `,
					...
					if (newGroupPosts && newGroupPosts.length > 0) {
						...
						const postsByGroup = newGroupPosts.reduce(
			...
			.on(
              ...
              `,
						...
						if (
							...
							const senderName =
								otherUser?.name || otherUser?.username || "Someone";
			...
			.on(
              ...
              `,
						...
						if (post && post.author_id === session.user.id) {
							...
							const authorName =
								(replyAuthor as any)?.name ||
								(replyAuthor as any)?.username ||
								"Someone";
			...
			.on(
				...
				async (payload) => {
					...
					if (
						...
						// Make sure it's a post vote, not reply vote
						...
						if (post && post.author_id === session.user.id) {
							...
							const voterName =
								(voter as any)?.name || (voter as any)?.username || "Someone";
			...
			.on(
              ...
              `,
						...
						if (group && group.creator_id === session.user.id) {
							...
							const memberName =
								(newMember as any)?.name ||
								(newMember as any)?.username ||
								"Someone";
		...
		const postsChannel = supabase
			...
			.on(
                ...
                `,
							...
							if (groupAndAuthor) {
								const authorName =
									(groupAndAuthor.author as any)?.name ||
									(groupAndAuthor.author as any)?.username ||
									"Someone";
								const groupName =
									(groupAndAuthor.group as any)?.name || "Unknown Group";
