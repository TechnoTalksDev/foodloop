...
const getNotificationTypeColor = (type: string) => {
  switch (type) {
    ...
    case 'community_reply': return '#22c55e'; // Green
    case 'community_vote': return '#f97316'; // Orange
    case 'group_join': return '#3b82f6'; // Blue
    case 'group_update': return '#6366f1'; // Indigo
...
const getNotificationTypeLabel = (type: string) => {
  switch (type) {
    ...
    case 'community_reply': return 'Reply';
    case 'community_vote': return 'Vote';
    case 'group_join': return 'Group';
    case 'group_update': return 'Group';
...
export default function NotificationModal() {
  ...
  const filteredNotifications = notifications.filter(n => {
    ...
    if (filter === 'community') {
      return ['community_post', 'community_reply', 'community_vote', 'group_join', 'group_update'].includes(n.type);
    }
    return true; // 'all'
  });

  const communityNotificationCount = notifications.filter(n => 
    ['community_post', 'community_reply', 'community_vote', 'group_join', 'group_update'].includes(n.type)
  ).length;
  ...
  const renderNotificationSettings = () => (
    <View className="p-4 border-t border-border">
      ...
      <ScrollView className="max-h-96" showsVerticalScrollIndicator={false}>
        {/* Community Section */}
        <View className="mb-6">
          <Text className="text-base font-semibold mb-3 text-primary">Community Notifications</Text>
          
          <View className="flex-row items-center justify-between py-3 border-b border-border">
            <View className="flex-1">
              <Text className="font-medium">Community Updates</Text>
              <Text className="text-sm text-muted-foreground">Popular posts and trending discussions</Text>
            </View>
            <TouchableOpacity
              onPress={() => updatePreferences({ community_updates: !preferences.community_updates })}
            ...
            >
              <View 
                className={`w-5 h-5 rounded-full bg-white shadow-sm ${
                  preferences.community_updates ? 'self-end mr-0.5' : 'self-start ml-0.5'
                }`}
              />
            </TouchableOpacity>
          </View>
          ...
          <View className="flex-row items-center justify-between py-3 border-b border-border">
            <View className="flex-1">
              ...
              <Text className="text-sm text-muted-foreground">When someone replies to your posts</Text>
            </View>
            <TouchableOpacity
              onPress={() => updatePreferences({ community_replies: !preferences.community_replies })}
              className={`w-12 h-6 rounded-full justify-center ${
            ...
            >
              <View 
                className={`w-5 h-5 rounded-full bg-white shadow-sm ${
                  preferences.community_replies ? 'self-end mr-0.5' : 'self-start ml-0.5'
                }`}
              />
            </TouchableOpacity>
          </View>
          ...
          <View className="flex-row items-center justify-between py-3 border-b border-border">
            <View className="flex-1">
              ...
              <Text className="text-sm text-muted-foreground">When someone upvotes your content</Text>
            </View>
            <TouchableOpacity
              onPress={() => updatePreferences({ community_votes: !preferences.community_votes })}
              className={`w-12 h-6 rounded-full justify-center ${
            ...
            >
              <View 
                className={`w-5 h-5 rounded-full bg-white shadow-sm ${
                  preferences.community_votes ? 'self-end mr-0.5' : 'self-start ml-0.5'
                }`}
              />
            </TouchableOpacity>
          </View>
          ...
          <View className="flex-row items-center justify-between py-3 border-b border-border">
            <View className="flex-1">
              ...
              <Text className="text-sm text-muted-foreground">New posts and members in your groups</Text>
            </View>
            <TouchableOpacity
              onPress={() => updatePreferences({ group_updates: !preferences.group_updates })}
              className={`w-12 h-6 rounded-full justify-center ${
            ...
            >
              <View 
                className={`w-5 h-5 rounded-full bg-white shadow-sm ${
                  preferences.group_updates ? 'self-end mr-0.5' : 'self-start ml-0.5'
                }`}
              />
            </TouchableOpacity>
          </View>
        ...
        {/* Other notifications */}
        <View className="mb-6">
          <Text className="text-base font-semibold mb-3 text-primary">Other Notifications</Text>
          
          {Object.entries(preferences).filter(([key]) => 
            !['community_updates', 'community_replies', 'community_votes', 'group_updates', 'quiet_hours_enabled', 'quiet_hours_start', 'quiet_hours_end'].includes(key)
          ).map(([key, value]) => {
      ...
      <View className="flex-row items-center justify-between px-4 py-3 border-b border-border">
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <View className="flex-row bg-secondary/30 rounded-lg p-1">
            ...
            >
              <Text className={`font-medium ${
                filter === 'community' ? 'text-primary-foreground' : 'text-foreground'
              }`}>
          ...
          <View className="flex-1 items-center justify-center p-8">
            <View className="items-center">
              <Text className="text-6xl mb-4">
                {filter === 'community' ? '👥' : filter === 'unread' ? '✅' : '🔔'}
              </Text>
