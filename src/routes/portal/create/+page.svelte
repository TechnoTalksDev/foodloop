<script lang="ts">
	...
	const options = {
		classes: {
			...
			input:
				'border-1 w-full rounded-md border-0 shadow-sm px-4 py-3 pl-10 pr-20 text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 sm:text-sm',
	...
	<div class="container mx-auto px-4">
		<div class="mx-auto max-w-md">
			<form method="POST" action="?/create" enctype="multipart/form-data">
				<Card.Root class="w-full overflow-hidden rounded-xl border-0 shadow-lg">
					<Card.Header class="border-b border-green-100 bg-green-50">
						...
						<Card.Description class="text-green-800"
							>List your surplus food item for others to rescue.</Card.Description
						>
					</Card.Header>
						...
						<div class="grid w-full items-center gap-5">
							<div class="flex flex-col space-y-2">
								...
								<Input
									...
									required
									bind:value={name}
									class="rounded-lg border-gray-300 focus:border-green-500 focus:ring focus:ring-green-200 focus:ring-opacity-50"
								/>
							</div>
							...
							<div class="flex flex-col space-y-2">
								...
								<Textarea
									...
									class="min-h-24 rounded-lg border-gray-300 focus:border-green-500 focus:ring focus:ring-green-200 focus:ring-opacity-50"
								/>
							</div>
							...
							<div class="flex flex-col space-y-2">
								...
								<input
									...
									id="location"
									name="location"
									value={fullResponse?.formattedAddress || ''}
									required
								/>
								<PlaceAutocomplete
									{onResponse}
									...
									{options}
									{requestParams}
									PUBLIC_GOOGLE_MAPS_API_KEY={maps_api}
								/>
								<p class="mt-1 text-sm text-green-600">{fullResponse?.formattedAddress || ''}</p>
							</div>
							...
							<div class="flex flex-col space-y-2">
								...
								<div
									class="flex items-center rounded-lg border-2 border-dashed border-green-200 bg-green-50 p-4"
								>
									<label
										for="picture"
										class="mr-3 cursor-pointer rounded bg-green-600 px-4 py-2 text-white hover:bg-green-700"
										>Choose File</label
									>
					...
					<Card.Footer class="flex justify-between border-t border-gray-100 bg-gray-50 p-6">
						<Button
							variant="outline"
							type="button"
