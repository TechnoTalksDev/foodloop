...
import { Group, Post, PostReply, CreateGroupData, CreatePostData, CreateReplyData } from '@/types/community';

// Groups hooks
export const useGroups = (filters?: { category?: string; featured?: boolean }) => {
...
export const useCreatePost = () => {
  ...
  const createPost = async (data: CreatePostData): Promise<Post | null> => {
    try {
      ...
      if (data.group_id) {
        await supabase.rpc('increment_group_post_count', { group_id: data.group_id });
      }
...
export const usePostVoting = () => {
  ...
  const votePost = async (postId: number, voteType: 'up' | 'down'): Promise<boolean> => {
    try {
      setLoading(true);
      const { data: user } = await supabase.auth.getUser();
...
export const useCreateReply = () => {
  ...
  const createReply = async (data: CreateReplyData): Promise<PostReply | null> => {
    try {
      setLoading(true);
      setError(null);
