...
export function useAchievements() {
	...
	const getNextMilestone = (): NextMilestone | null => {
		const incompleteAchievements = userAchievements.filter(
			(ua) => !ua.is_completed,
		);
		...
		const nextAchievement = incompleteAchievements.reduce(
			(closest, current) => {
				const currentProgress =
					(current.current_progress / current.achievement.target_value) * 100;
				const closestProgress =
					(closest.current_progress / closest.achievement.target_value) * 100;
		...
		const progressPercentage = Math.min(
			(nextAchievement.current_progress /
				nextAchievement.achievement.target_value) *
				100,
			100,
		);
