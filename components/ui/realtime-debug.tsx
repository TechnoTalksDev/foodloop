		...
		const testChannel = supabase
			...
			.subscribe((status) => {
				...
				if (status === "SUBSCRIBED") {
					...
					Alert.alert(
						"❌ Error",
						"Realtime connection failed. Check your Supabase publications settings.",
					);
				}
			});
		...
		const messageTestChannel = supabase
			...
			.on(
				...
				(payload) => {
					...
					Alert.alert(
						"🎉 Message Received!",
						`Got realtime message: "${payload.new.content}"`,
					);
				},
			)
			.subscribe((status) => {
				...
				if (status === "SUBSCRIBED") {
					Alert.alert(
						"✅ Subscribed",
						"Now listening for messages. Send a message to test!",
					);
				}
			});
